...
public class CourseStudentServiceImpl extends BaseServiceImpl<CourseStudent> implements CourseStudentService {
    ...
    @Override
    public PageInfo<CourseStudentDTO> getCourseStudentsPage(Long courseId, Integer pageIndex, Integer pageSize,
            String keyword) {
        return PageHelper.startPage(pageIndex, pageSize)
                .doSelectPageInfo(() -> courseStudentMapper.getCourseStudentsWithPage(courseId, keyword));
    }
    ...
    @Override
    public Boolean isStudentEnrolled(Long courseId, Integer studentId) {
        CourseStudent courseStudent = courseStudentMapper.selectByCourseIdAndStudentId(courseId, studentId);
        return courseStudent != null && courseStudent.getStatus() == 1; // 状态为1表示已选
    }
}