...
public class TeacherCourseServiceImpl implements TeacherCourseService {
    ...
    public void updateStudentScore(CourseStudentRequestVM requestVM) {
        // 验证课程和学生关联是否存在
        if (courseStudentMapper.selectByCourseIdAndStudentId(requestVM.getCourseId(), requestVM.getStudentId()) == null) {
            throw new RuntimeException("该学生未选择此课程");
        }
        
        // 更新学生成绩
        courseStudentMapper.updateFinalScore(requestVM.getCourseId(), requestVM.getStudentId(), requestVM.getFinalScore());
    }
